...
class SimpleDrawer extends React.PureComponent<{ windowSize: Media.TWindowSize, changeWindowSize: (windowSize: Media.TWindowSize) => void }> {
  ...
  async changeVisible(visible: boolean) {
    if (this.animPromise) this.animPromise.abort()
    if (visible) {
    ...
    this.animPromise = new DrawerPromise({ ...this.promisePars, windowSize: this.props.windowSize, visible })
    this.animPromise.start()
