...
export const View: React.SFC<ViewProperties> = props => {
  const { style, web = {} as React.HTMLAttributes<any>, webStyle, ...otherPropsTyped } = props
  const { style: webStyle2, className = '', ...webRest } = web
  return <div {...otherPropsTyped as any} {...webRest} className={className + ' ' + renderCSSs({ display: 'flex', flexDirection: 'column' }, style as any, webStyle2, webStyle)} />
}
