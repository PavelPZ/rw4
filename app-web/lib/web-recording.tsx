...
const provider: React.SFC<Recording.IProps> = props => {
  ...
  const btn = <Button onClick={props.changeSize} floating secondary fixed fixedPosition={'bl'} className={renderCSS({ marginLeft: -15, zIndex: 21 })}><Icon name={GUI.IonicNames.analytics} /></Button>
  ...
  return <div className={renderCSS({ position: 'relative', })}>
    <div className={renderCSS({ position: 'absolute', width: 200, left: 0, top: 0, bottom: 0, borderWidth: 1, borderStyle: 'solid', borderColor: 'lightgray' })}>
      <div className={renderCSS({ margin: 5 })}>
        {props.playLists && props.playLists.length > 0 ? <Btn icon={GUI.IonicNames.analytics} title='INVERT' click={props.listInvert} /> : null}
        {props.playLists && props.playLists.find(l => l.checked) ? <Btn icon={GUI.IonicNames.remove} title='DELETE' click={props.listDelete} /> : null}
...
const Btn: React.SFC<{ icon: GUI.IonicNames; title: string; click: () => void }> = props => {
  return <Button mini primary className={renderCSS({ height: 40, padding: 5, minWidth: 0, display: 'inline-block', marginLeft: 5 })} onClick={props.click}>
    <Icon name={props.icon} style={{ fontSize: 18 }} /><br />
    <span className={renderCSS({ fontSize: 10 })}>{props.title}</span>
  </Button>
}

const PlayAllBtn: React.SFC<Recording.IProps> = props => {
  if (props.playLists && props.playLists.length > 0 && props.mode == Recording.TModes.no) return <Btn icon={GUI.IonicNames.fastforward} title='PLAY ALL' click={() => props.playStart(allSelected(props.playLists.length)/*Recording.Consts.playAllPlaylist*/)} />
  if (props.mode == Recording.TModes.playing) return <Btn icon={GUI.IonicNames.close} title= 'CANCEL' click= { props.playCancel } />
  return null
}
...
const PlaySelectedBtn: React.SFC<Recording.IProps> = props => {
  ...
  if (selected.length > 0) return <Btn icon={GUI.IonicNames.fastforward} title='PLAY' click={() => props.playStart(selected)} />
  return null
}
...
const PlayRecordingBtn: React.SFC<Recording.IProps> = props => {
  if (props.recording && props.recording.length > 0 && props.mode == Recording.TModes.no) return <Btn icon={GUI.IonicNames.play} title='PLAY' click={() => props.playStart(null)} />
  return null
}

const RecordBtn: React.SFC<Recording.IProps> = props => {
  if (props.mode == Recording.TModes.no) return <Btn icon={GUI.IonicNames.recording} title='RECORD' click={() => props.recordStart()} />
  if (props.mode == Recording.TModes.recording) return <Btn icon={GUI.IonicNames.stopwatch} title='STOP' click={props.recordEnd} />
  return null
}

const RecordSavedBtn: React.SFC<Recording.IProps> = props => {
  if (props.recording && props.recording.length > 0 && props.mode == Recording.TModes.no) return <Btn icon={GUI.IonicNames.star} title='SAVE' key='record' click={props.recordSave} />
  return null
}
...
const blockGui: React.SFC<BlockGui.IState> = props => props.state == BlockGui.State.no ? null : <Portal visible={props.state != BlockGui.State.no} className={renderCSS({ position: 'fixed', bottom: 0, right: 0, top: 0, left: 0, display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 99, cursor: 'wait' })} >
