...
import { View, Text, Animated, AnimatedView, Swiper } from '../gui/gui'
import { AnimatedMobileDrawer } from '../../app-web/gui/view'
...
const mobile: React.SFC<Drawer.IDispatchProps & Drawer.IStateProps & Drawer.IOwnProps> = props => {
  ...
  const absolute = { ...absoluteStretch as any, marginTop: window.lmGlobal.topMargin } as any
  return <AnimatedMobileDrawer
    targetVisible={drawerVisible}
    hideDrawer={() => showDrawer(false)}
    content={<Content {...rest} {...content} />}
    menu={<Menu {...rest} {...menu} />}
    drawerWidth={drawerWidth}
    visibleOpacity={[0, 0.85]}
    visibleLeft={[-drawerWidth, 0]}
  />
}
...
const tablet: React.SFC<Drawer.IDispatchProps & Drawer.IStateProps & Drawer.IOwnProps> = props => {
  ...
  const { content, menu, ...rest } = props
  return <AnimatedView anim={{ propName: 'left', targetValue: drawerVisible ? 0 : -nonMobileMenuWidth }} style={{ position: 'absolute', top: 0, right: 0, bottom: 0, marginTop: window.lmGlobal.topMargin, flexDirection: 'row', }}>
