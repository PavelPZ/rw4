...
export class AnimatedTabletDrawer extends React.PureComponent<GUI.IAnimatedMobileDrawerProps> {
  rendered: boolean
  ...
  render() {
    ...
    const { duration, content, menu, willBeVisible, drawerWidth, doShowDrawer, screenWidth } = props
    ...
    return <View style={absoluteStretch as any} {...!willBeVisible ? toVisibleHandlers.panHandlers : toHideHandlers.panHandlers}>
      <Animated.View style={{ left: value as any, position: 'absolute', top: 0, right: 0, bottom: 0, flexDirection: 'row' }} >
...
export class AnimatedTabletDrawer2 extends React.Component<GUI.IAnimatedMobileDrawerProps> {
  rendered: boolean
  value = new Animated.Value(0)
  animation: ReactNative.Animated.CompositeAnimation
  ...
  render() {
    ...
    if (!rendered)
      ...
      this.animation = Animated.timing(value, { toValue: willBeVisible ? 0 : -drawerWidth, duration: duration || App.Consts.animationDurationMsec, delay: 1 })
      this.animation.start(() => this.animation = null)
    }

    return <Animated.View style={{ left: value as any, position: 'absolute', top: 0, right: 0, bottom: 0, flexDirection: 'row', }}>
      {React.cloneElement(menu, { ...menu.props, key: 1, style: { ...menu.props.style, width: drawerWidth } })}
      {React.cloneElement(content, { ...content.props, key: 0, style: { ...content.props.style, flex: 1 } })}
    </Animated.View>
  }
...
export class AnimatedMobileDrawer extends React.PureComponent<GUI.IAnimatedMobileDrawerProps> {
  ...
  render() {
    ...
    if (animation) { animation.stop(); this.animation = null }
    const opacityValue = value.interpolate({ inputRange: [0, 0.1, 1], outputRange: [0, 0, 0.85] })
    const leftValue = value.interpolate({ inputRange: [0, 1], outputRange: [-drawerWidth, 0] })
    const widthValue = value.interpolate({ inputRange: [0, 0.1, 1], outputRange: [0, screenWidth, screenWidth] })
    if (!rendered)
      this.rendered = true
    else {
      doAnimation(willBeVisible)
    ...
    return <View style={absoluteStretch as any} {...!willBeVisible ? toVisibleHandlers.panHandlers : toHideHandlers.panHandlers}>
      ...
      <Animated.View key={2} style={{ left: leftValue as any, ...topBottom, width: drawerWidth, display: 'flex' }} >
