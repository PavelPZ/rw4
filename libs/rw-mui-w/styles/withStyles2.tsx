...
function beforeWithStyleMui<C, R extends Mui2.TypedRules>(Component: Mui2.origComponentType<C, keyof R>) {
  ...
  const res: Mui2.SFC<C, R> = (props: Mui2.Props<{}, Mui2.TypedRules>) => {
    ...
    const classes = stylesToClassNames(expanRulesWeb(inlineClasses) as Mui2.ExpandedRulesWeb<R>)
    const webProps = { style: expanRuleWeb(style), classes, ...rest } as (C & Mui2.origStyledComponentProps<TKey>)
...
function expanRuleWeb<T extends Mui2.NativeStyle>(rule: Mui2.Rule<T>): Mui2.ExpandedRule<T> {
  if (!rule) return null
  const { web, native, ...rest } = rule
  return { ...rest, ...web }
}

function expanRulesWeb<R extends Mui2.TypedRules>(rules: Mui2.Rules<R>): Mui2.ExpandedRules<R> {
  if (!rules) return null
  const res: Mui2.ExpandedRules<R> = {}
  for (const p in rules) res[p] = expanRuleWeb(rules[p])
  return res
}
